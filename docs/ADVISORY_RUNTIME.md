...
`stem advisory call` is different. It is the explicit runtime boundary where a downstream provider adapter may be invoked. In v1.6.8, this mode exists to make the boundary visible and enforce its security contract before real provider execution is attached.

---

## What `stem advisory call` Does In v1.6.8
...
v1.6.8 does **not** ship a live provider adapter.
