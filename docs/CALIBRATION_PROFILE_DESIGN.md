...
Version: 1.7.5
Status: implemented mirror-only calibration contract with derive/simulate preview surfaces; 1.7.5 preview hardening complete; authoritative read-through remains future work
...
As of `1.7.5`, the repository ships a real calibration architecture:
...
In `1.7.5`, most score-affecting values are still implemented as runtime constants plus prose in `SCORING_RATIONALE.md`, even though mirror-only profile metadata, CLI-visible profile selection, and derive/simulate preview surfaces are already live. That is acceptable for the current release line, but it still creates a long-term maintenance risk:
...
This is the active shipped schema family in `1.7.5`.
...
- `stage_3_policy.b2_partial_credit_mode` is currently a declared mirror-only profile field; authoritative Stage 3 B2 scoring in `1.7.5` still follows the hardcoded scanner path and does not yet read this value directly
...
This narrow table is intentional. It keeps the translation layer visible, reviewable, and testable without pretending that every strong posture already has a release-grade named profile. In particular, `reproducibility_first` remains deferred in `1.7.5`; high reproducibility answers still fall back to `preview_only` Stage 4 emphasis rather than a named recommendation.
...
These are active preview-only deltas in `1.7.5`. They are not hidden similarity operations and they do not mutate the authoritative scan path.
...
- `documentation_lenient`
  - not active in the `1.7.5` rule table
- `research_repo_baseline`
  - not active in the `1.7.5` rule table
- `biosecurity_cautious`
  - not active in the `1.7.5` rule table
