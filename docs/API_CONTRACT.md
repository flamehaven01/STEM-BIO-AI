...
| `stem_ai_version` | string | Package version (e.g. `"1.7.5"`) |
...
| `calibration_profile.profile_read_mode` | string | `"mirror_only"` in `1.7.5`; later `"authoritative"` when scan scoring reads policy values directly |
...
In `1.7.5`, `scan --policy <name>` still keeps authoritative scan scoring on the deterministic runtime-constant path. Policy selection changes surfaced metadata only; governed score-delta preview belongs to `stem policy simulate`.
