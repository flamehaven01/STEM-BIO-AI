...
| `stem_ai_version` | string | Package version (e.g. `"1.8.4"`) |
...
| `calibration_profile.profile_read_mode` | string | `"mirror_only"` in `1.8.4`; later `"authoritative"` when scan scoring reads policy values directly |
...
In `1.8.4`, `scan --policy <name>` still keeps authoritative scan scoring on the deterministic runtime-constant path. Policy selection changes surfaced metadata only; governed score-delta preview belongs to `stem policy simulate`.
