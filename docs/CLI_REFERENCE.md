...
The 1.7.5 CLI is organized around workflows instead of one long option string, exposes named calibration profiles directly, and adds auditable researcher-intent derive/simulate surfaces.
...
Policy selection is mirror-only in 1.7.5: the chosen profile is surfaced in outputs and summaries, and derive/simulate preview lanes can compare postures, but authoritative score computation still follows the current deterministic runtime constants.
Policy selection remains mirror-only in scans in 1.7.5. The new `policy derive` and `policy simulate` commands preview governed posture changes without turning those deltas into authoritative scan behavior.
...
Current `1.7.5` rule scope is intentionally narrow:
