...
stem-ai/
  ...
  memory/                     <-- MICA v0.2.4 memory layer (load first)
    ...
    stem-ai.mica.v1.7.8.json  <-- active archive (selected by mica.yaml)
    stem-ai-playbook.v1.7.8.md <-- active session protocol (selected by mica.yaml)
    stem-ai-lessons.v1.7.8.md  <-- active lessons history (selected by mica.yaml)
...
0. **Load MICA memory layer first (before any audit work):**
   ...
   - Report: `[MICA READY] stem-ai-bio v1.7.8 | mode: protocol_evolution | invariants: 18 active | pct: CLOSED`
