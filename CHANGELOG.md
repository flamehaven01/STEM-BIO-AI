...
- **Packaging build broke on every MICA rotation.** `python -m build` failed because the packaging manifests hard-coded versioned memory filenames that drifted from the active files (`MANIFEST.in` still on `v1.7.4`, `pyproject.toml` `data-files` on `v1.7.8`, package-data on `v1.8.3`). Root-cause hardening: `MANIFEST.in` now uses a version-agnostic `recursive-include memory *.json *.md *.yaml` glob, and a new `test_packaging_data_files_all_exist` regression asserts every `[tool.setuptools.data-files]` entry exists on disk — turning this recurring build break into a fast unit-test failure that names the stale entry. All active manifest entries point at the `v1.8.4` trio. (`data-files` cannot glob, so the guard test is the safety net for the wheel path.)
...
- Rotated the active package, CLI, documentation, policy, and MICA version surfaces to `v1.7.6`.
