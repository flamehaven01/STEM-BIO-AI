...
# Version: 1.8.1 | MICA Contract: 0.2.4 | Updated: 2026-06-05
...
3. Load memory/stem-ai-lessons.v1.8.1.md -- on demand for edge case disambiguation
...
7. Report: [MICA READY] stem-ai-bio v1.8.1 | mode: protocol_evolution | invariants: 18 active | pct: CLOSED
```
...
v1.8.1 keeps the active deterministic scanner surface and the workflow-oriented CLI UX from v1.6.2,
...
- Policy selection and derive/simulate outcomes remain **mirror-only** in 1.8.1. The active profile and preview posture are visible and testable, but authoritative score computation still follows the current deterministic runtime constants.
...
4. If new failure mode discovered: add to `memory/stem-ai-lessons.v1.8.1.md` per L-ADD protocol.
...
| playbook | `memory/stem-ai-playbook.v1.8.1.md` |
| archive | `memory/stem-ai.mica.v1.8.1.json` |
...
5. **Recording new drift evidence:** Append to `memory/stem-ai-lessons.v1.8.1.md` with:
