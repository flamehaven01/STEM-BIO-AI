...
# Version: 1.6.2 | Updated: 2026-05-08
...
3. Load memory/stem-ai-lessons.v1.6.2.md -- on demand for edge case disambiguation
4. Confirm IMMUTABLE rules are loaded from spec/STEM-AI_v1.1.2_CORE.md
5. Run PCT self-tests (see §3 below)
6. Report: [MICA READY] stem-ai-bio v1.6.2 | mode: protocol_evolution | invariants: 18 active
```
...
v1.6.2 keeps the active deterministic scanner surface and adds workflow-oriented CLI UX,
compact gate/advisory summaries, clearer `scan / gate / advisory` command separation,
and backward-compatible shortcuts for existing `stem <folder>` and `stem audit` usage.
...
4. If new failure mode discovered: add to `memory/stem-ai-lessons.v1.6.2.md` per L-ADD protocol.
...
| playbook | `memory/stem-ai-playbook.v1.6.2.md` |
| archive | `memory/stem-ai.mica.v1.6.2.json` |
...
5. **Recording new drift evidence:** Append to `memory/stem-ai-lessons.v1.6.2.md` with:
