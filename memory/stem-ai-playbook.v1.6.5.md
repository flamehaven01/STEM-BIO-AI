...
# Version: 1.6.5 | Updated: 2026-05-10
...
3. Load memory/stem-ai-lessons.v1.6.5.md -- on demand for edge case disambiguation
4. Confirm IMMUTABLE rules are loaded from spec/STEM-AI_v1.1.2_CORE.md
5. Run PCT self-tests (see §3 below)
6. Report: [MICA READY] stem-ai-bio v1.6.5 | mode: protocol_evolution | invariants: 18 active
```
...
v1.6.5 keeps the active deterministic scanner surface and the workflow-oriented CLI UX from v1.6.2,
and adds two patch-level correctness refinements plus taxonomy provenance surfacing:

- Stage 3 B1 provenance scoring now distinguishes negative or non-approval IRB / data-source contexts from positive provenance evidence.
- AST file-limit conditions are now surfaced directly in stdout, Markdown, and PDF outputs so reviewers can see when C1/C4 AST-backed analysis used a capped subset.
- Stage 3 B2 now requires structured boundary language before partial credit is awarded, and result objects expose the active CA taxonomy version/source.
...
4. If new failure mode discovered: add to `memory/stem-ai-lessons.v1.6.5.md` per L-ADD protocol.
...
| playbook | `memory/stem-ai-playbook.v1.6.5.md` |
| archive | `memory/stem-ai.mica.v1.6.5.json` |
...
5. **Recording new drift evidence:** Append to `memory/stem-ai-lessons.v1.6.5.md` with:
