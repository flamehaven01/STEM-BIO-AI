...
3. Load memory/stem-ai-lessons.v1.6.6.md -- on demand for edge case disambiguation
4. Confirm IMMUTABLE rules are loaded from spec/STEM-AI_v1.1.2_CORE.md
5. Run PCT self-tests (see §3 below)
6. Report: [MICA READY] stem-ai-bio v1.6.6 | mode: protocol_evolution | invariants: 18 active
```
...
v1.6.6 keeps the active deterministic scanner surface and the workflow-oriented CLI UX from v1.6.2,
retains the 1.6.3/1.6.4/1.6.5 scoring and calibration-contract refinements, and adds policy-visibility surfaces:
...
- Policy selection remains **mirror-only** in 1.6.6. The selected profile is visible and testable, but score computation still follows the current deterministic runtime constants.
...
4. If new failure mode discovered: add to `memory/stem-ai-lessons.v1.6.6.md` per L-ADD protocol.
...
| playbook | `memory/stem-ai-playbook.v1.6.6.md` |
| archive | `memory/stem-ai.mica.v1.6.6.json` |
...
5. **Recording new drift evidence:** Append to `memory/stem-ai-lessons.v1.6.6.md` with:
