...
# Version: 1.7.4 | Updated: 2026-05-13
...
3. Load memory/stem-ai-lessons.v1.7.4.md -- on demand for edge case disambiguation
4. Confirm IMMUTABLE rules are loaded from spec/STEM-AI_v1.1.2_CORE.md
5. Run PCT self-tests (see §3 below)
6. Report: [MICA READY] stem-ai-bio v1.7.4 | mode: protocol_evolution | invariants: 18 active
```
...
v1.7.4 keeps the active deterministic scanner surface and the workflow-oriented CLI UX from v1.6.2,
...
- Policy selection and derive/simulate outcomes remain **mirror-only** in 1.7.4. The active profile and preview posture are visible and testable, but authoritative score computation still follows the current deterministic runtime constants.
...
4. If new failure mode discovered: add to `memory/stem-ai-lessons.v1.7.4.md` per L-ADD protocol.
...
| playbook | `memory/stem-ai-playbook.v1.7.4.md` |
| archive | `memory/stem-ai.mica.v1.7.4.json` |
...
5. **Recording new drift evidence:** Append to `memory/stem-ai-lessons.v1.7.4.md` with:
