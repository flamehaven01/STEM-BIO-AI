...
3. Load memory/stem-ai-lessons.v1.5.7.md -- on demand for edge case disambiguation
4. Confirm IMMUTABLE rules are loaded from spec/STEM-AI_v1.1.2_CORE.md
5. Run PCT self-tests (see §3 below)
6. Report: [MICA READY] stem-ai-bio v1.5.7 | mode: protocol_evolution | invariants: 18 active
```
...
v1.5.7 keeps the active deterministic scanner surface and adds advisory contract hardening,
provider-request argument validation, exported contract schemas, and packet self-checks:
...
4. If new failure mode discovered: add to `memory/stem-ai-lessons.v1.5.7.md` per L-ADD protocol.
...
| playbook | `memory/stem-ai-playbook.v1.5.7.md` |
| archive | `memory/stem-ai.mica.v1.5.7.json` |
...
5. **Recording new drift evidence:** Append to `memory/stem-ai-lessons.v1.5.7.md` with:
